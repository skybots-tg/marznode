...
def get_x25519(xray_path: str, private_key: str = None) -> Dict[str, str] | None:
    ...
    try:
        ...
        logger.warning(
            "Could not parse x25519 output; known labels missing. Raw output:\n%s",
            output,
        )
        return None
    except subprocess.CalledProcessError as e:
        logger.error(
            "Error executing `xray x25519`: %s. Output: %s",
            e,
            e.output.decode("utf-8", errors="replace") if e.output else "N/A",
        )
        return None
